...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeQpaycoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeQpaycoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeQpaycoinAmount = strAnonymizeQpaycoinAmount.remove(strAnonymizeQpaycoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeQpaycoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeQpaycoinAmount * COIN) nMaxToAnonymize = nAnonymizeQpaycoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeQpaycoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeQpaycoinAmount));
        strAnonymizeQpaycoinAmount = strAnonymizeQpaycoinAmount.remove(strAnonymizeQpaycoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeQpaycoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
